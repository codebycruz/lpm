...
local process2 = require("process2")
...
test.it("exec captures stderr (merged into stdout on posix)", function()
	...
	local code, stdout, stderr = process2.exec(sh, { shc, cmd })
	test.equal(code, 0)
...
test.it("exec passes stdin", function()
	...
	local code, stdout = process2.exec(sh, { shc, cmd }, { stdin = "hello" })
	test.equal(code, 0)
	test.truthy(stdout and stdout:find("hello"))
end)
...
test.it("exec passes env vars", function()
	...
	local code, stdout = process2.exec(sh, { shc, cmd }, { env = { MY_VAR = "testval" } })
	test.equal(code, 0)
...
test.it("exec respects cwd", function()
	...
	local code, stdout = process2.exec(sh, { shc, cmd }, { cwd = tmpdir })
	test.equal(code, 0)
...
test.it("exec handles args with spaces and special chars", function()
	...
	local code = process2.exec(sh, { shc, isWindows and 'echo "hello world"' or "printf '%s' 'hello world'" })
	test.equal(code, 0)
end)
...
test.it("spawn returns a Child with a pid", function()
	local child, err = process2.spawn(sh, { shc, "exit 0" })
...
test.it("spawn Child:wait returns exit code", function()
	local child = process2.spawn(sh, { shc, "exit 42" })
	test.truthy(child)
...
test.it("spawn Child:wait captures stdout when piped", function()
	...
	local child = process2.spawn(sh, { shc, cmd }, { stdout = "pipe" })
	test.truthy(child)
...
test.it("spawn Child:kill terminates the process", function()
	...
	local child = process2.spawn(sh, { shc, cmd })
	test.truthy(child)
	child:kill(true)
